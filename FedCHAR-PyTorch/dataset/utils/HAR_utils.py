...
# =======================================================================================
def split_data(X, y, min=None):
    # Split dataset
    train_data, test_data = [], []
    num_samples = {'train':[], 'test':[]}
    for i in range(len(y)):
        if min != None:
            num_selected = np.random.randint(min, y[i].shape[0]+1) # [min, max)
            selected_idx = np.random.choice(np.arange(y[i].shape[0]), num_selected, replace=False)
            X[i] = X[i][selected_idx]
            y[i] = y[i][selected_idx]
