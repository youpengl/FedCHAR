...
class Model(object):
    ...
    def create_model(self, q, optimizer):
        ...
        features = tf.placeholder(tf.float32, shape=[None, TMAX*32*32*1], name='features')
        labels = tf.placeholder(tf.int64, shape=[None, ], name='labels')
        input_layer = tf.reshape(features, [-1, TMAX, 32, 32, 1])
        input_shape = (TMAX, 32, 32, 1)
        # Feature extraction part
        x = TimeDistributed(Conv2D(8, kernel_size=(5, 5), activation='relu', data_format='channels_last', input_shape=input_shape))(input_layer)  # (@,T_MAX,20,20,1)=>(@,T_MAX,16,16,16)
        x = TimeDistributed(MaxPooling2D(pool_size=(2, 2), strides=2))(x)
        x = TimeDistributed(Conv2D(16, kernel_size=(5, 5), activation='relu', data_format='channels_last'))(x)  # (@,T_MAX,20,20,1)=>(@,T_MAX,16,16,16)
        x = TimeDistributed(MaxPooling2D(pool_size=(2, 2), strides=2))(x)
